...
describe("iteration-05", () => {
  it("Should set the document title to the name when mounted", () => {
    const { unmount } = render(<Greeter name="Assembler" emoji="🖐" />);
    expect(document.title).toBe("Hello Assembler 🖐");
    unmount();
    expect(document.title).toBe("Bye Assembler 🖐");
  });

  // Scape test as it was just to illustrate how lifecycles work in iteration 05
  xit("Should set the document title to the name when rerendered", () => {
